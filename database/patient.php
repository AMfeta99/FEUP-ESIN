...
function  getPatientNotification($patient_id){
    global $dbh;
    $stmt = $dbh->prepare("SELECT message, ReceiveNotification.id as id, Reservation.id as reservation, Doctor.name as doctor, Department.name as department, Reservation.date as date, Reservation.time as time
